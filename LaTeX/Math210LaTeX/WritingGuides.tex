...
\item In professional writing (as in this class), the third person is usually pre-
ferred over the second, and slightly over the first, but avoiding artificial
...
\begin{itemize}
\item For each homework problem, you should first state the essential question being asked. You do not have to restate definitions, set-up, etc. For example, if the problem in the book states ``\#4. A set $X$ is said to be countable if it can be put in 1-1 correspondence with the natural numbers. Let $A=\mathbb{Z}_7$ and $B$ be a countable set. Show that a function $f:A \to B$ is not onto.'' then the first thing you should write is ``\#4.  Show that a function $f:A \to B$ is not onto.'' Then start your proof in a new paragraph.
\end{itemize}

\section{Audience Issues}
\begin{itemize}
\item You will, except when otherwise instructed, be writing with me, the teacher, as your audience. However, the level of your writing should be more that of an ``open letter,''\footnote[1]{The ``open letter'' is a rhetorical form in which one writes a letter, ostensibly to one person
(usually a high official), but which is published more broadly. Examples may sometimes be
seen in the opinion section of a newspaper, where a person writes a letter to the Governor
or the President, but also sends a copy to the newspaper, so that it be may be read by the
broader public.} in the sense that your classmates should be
able to read it with total understanding. You may assume that your audience understands the material from prerequisite courses and the present
...
\begin{itemize}
\item All assignments should be neatly and professionally presented. Papers should be reasonably clean, and should be stapled, paper-clipped, or otherwise neatly fastened.
